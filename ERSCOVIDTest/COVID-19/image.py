...
def adjust_transform_for_image(transform, image, relative_translation):
    ...
    zz, width, height, channels = image.shape
...
def apply_transform(matrix, image, params):
    ...
    """
    #z, y, x
    #print('image/apply_transform, image.shape', image.shape)
    if image.shape[-1] > 1:
        multiChannel = True
    else:
        multiChannel = False
    image = np.transpose(image[:,:,:,0], axes=(1, 2, 0))#[y, x, z]
    ...
    output = np.transpose(output, axes=(2, 0, 1))#for [y, x, z]
